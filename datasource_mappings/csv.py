...
class CSV:
    ...
    def set_csv_file(self):
        self.__df = pd.read_csv(self.__file_path, sep=r";", engine="python")
    ...
    def specify_domain_range(self, result, create_class, temp_class):
        ...
        target_domain = temp_string.replace(" ","")
            ...
            for col in self.__df:
                if col == val:
                    for i, instance in self.__df[col].items():
                        if (instance is not None) and (instance != 0) and (instance != 0.0):
...
def csv_to_ontology(file_path, onto):
    csv = CSV(file_path, onto, None, None)
    csv.set_csv_file()
    keywordsList = csv.pre_processing_csv()
    ...
    target_domain_class = csv.specify_domain_range(result, createClass, tempClass)
    if str(target_domain_class) in str(list(result.values())[0]):
        csv.add_instances_to_class(result, tempClass,target_domain_class)
        csv.add_instances_to_new_class(createClass,target_domain_class)
    else:
        csv.add_instances_to_new_class(createClass,target_domain_class)
        csv.add_instances_to_class(result, tempClass, target_domain_class)
    csv.save_ontology()